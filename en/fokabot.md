...
old_id: 4
---
These are the commands supported by RealistikBot, our BanchoBot equivalent..  
...
- `/np` - Show PP for the current playing song and select ut (only if is a osu! standard song)  
- `!last` - Show info (and gained PP, if it was an osu! standard score) about the last submitted score  
- `!with <mods>` - Show PP for the previous requested beatmap with requested mods. Supported mods are `NF, EZ, HD, HR, DT, HT, NC, FL, SO, RX, AP.`. Don't use spaces for multiple mods (eg: `!with HDHR`). Furthermore adding RX and AP modes will change the PP system being used, for example `!with RX` will show PP values for relax.
...
- `!system reload` - Reload bancho settings (editable from RealistikPanel)  
